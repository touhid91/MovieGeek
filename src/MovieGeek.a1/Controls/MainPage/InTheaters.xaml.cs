...
{
    public partial class InTheaters : UserControl
    {
        ...
        private async void UserControl_Loaded(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            if (IsolatedStorageHelper.FileExists(IsoDir.InTheaters))
            {
                await IsolatedStorageHelper.ReadAsync(IsoDir.InTheaters).ContinueWith(task =>
                {
                    movies = task.Result.CreateDocument().CleanUp().ParseTopBoxOffice();
            ...
            using (var client = new HttpClient())
            {
                await client.GetStringAsync(UriDictionary.InTheatersUri).ContinueWith(async task =>
                {
                    if (task.IsFaulted)
                        return;
                    movies = null;
                    movies = task.Result.CleanUp().CreateDocument().CleanUp().ParseTopBoxOffice();
                    if (!movies.Equals(null))
                    {
                        SynchronizationContext.Send(callback =>
                        {
